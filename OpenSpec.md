...
### CHANGE-A002 [ARCHIVED] — Legacy V1 climatology assets archived (scheduled for deletion)

- Date discovered: 2026-04-26 (during P-00.0 GEE asset folder verification)
- Date archived: 2026-04-26
- Initiator: Researcher + Claude Code (P-00.0 sanity check выявил несоответствие)
- Type: Asset structure cleanup (operational, не методология)
- Affected GEE assets:
  - **Source (deleted):** `RuPlumeScan/backgrounds/{CH4, CH4_05..10, NO2, NO2_03..10, SO2, SO2_03..10}` — 25 IMAGE-ассетов, лежавших как siblings прямо под `backgrounds/`.
  - **Destination (preserved):** `RuPlumeScan/_legacy_v1_archive/backgrounds/<same_names>` — все 25 IMAGE скопированы целиком.
- Affected code: `src/py/setup/archive_legacy_v1_backgrounds.py` (one-off скрипт, идемпотентен; включает `--purge` mode для финального удаления).
- Reason: 25 ассетов остались в проекте от V1-итерации (см. CHANGE-A001), которая использовала monthly composites + threshold approach — deprecated по DNA v2.1 §2.1. Эти assets:
  1. Построены по концептуально неверной методологии (composite median подавляет transient signal ~30×, см. CHANGE-A001).
  2. Не соответствуют Algorithm v2.2 §3.4 (regional climatology с industrial buffer exclusion + annulus correction).
  3. Имена `backgrounds/CH4`, `NO2`, `SO2` — IMAGE, а не Folder, что конфликтует с RNA §3.1 структурой `backgrounds/<gas>/climatology_2019_2025` (ожидается Folder).
- Alternatives considered:
  - A) Оставить in-place, P-01.0 пишет в новые имена (`climatology_2019_2025_v2.2`) — отвергнуто, конфликт с RNA structure.
  - B) Архивировать в `_legacy_v1_archive/backgrounds/` (плоско) — **выбрано**.
  - C) Удалить сразу — отвергнуто, теряется возможность показать «V1 deprecated vs V2.2 corrected» в Discussion tool-paper.
- Decision: copy → verify → delete originals → recreate `backgrounds/{CH4,NO2,SO2}` как Folder per RNA §3.1.
- Verification:
  - 25/25 IMAGE присутствуют в `_legacy_v1_archive/backgrounds/` (через `ee.data.listAssets`).
  - 0 IMAGE остались под `backgrounds/` (только 3 свежесозданных Folder).
  - Скрипт идемпотентен: повторный запуск пропускает уже архивированные.
- Scheduled deletion: после успешного завершения **Phase 1 (P-01.0)** с прохождением sanity check (Бованенково CH4 climatology baseline ~1880 ppb, industrial buffer exclusion работает над Norilsk/Кузбасс) **и** после публикации tool-paper в СПДЗЗ. До этого — sole purpose архива: возможность показать V1 deprecated approach side-by-side с V2.2 в Discussion section публикации (если решим).
- Purge command: `python -m setup.archive_legacy_v1_backgrounds --purge` (требует ручное подтверждение `'PURGE'`).
...
| 1.1 | 2026-04-26 | Добавлен CHANGE-A002 (archived legacy V1 climatology assets, 25 IMAGE moved to `_legacy_v1_archive/backgrounds/`, scheduled for deletion after Phase 1 + tool-paper). Discovered во время P-00.0 GEE asset verification. |
