...
### Actual ingestion measurements (2026-04-27)

```
zone_id              doc km²   meas km²   diff %   status
yugansky                 6500       6253   +3.8%   OK
verkhnetazovsky          6313       6294   +0.3%   OK
kuznetsky_alatau         4019       3999   +0.5%   OK
altaisky                 8810       8411   +4.5%   OK (within R2 tolerance)
```

**Worst diff: 4.5% (Алтайский) — все zones внутри R2 `<5%` tolerance, validation PASSED.**

#### Алтайский +4.5% diagnostic note

Алтайский имеет `optional_pending_quality` status — area mismatch здесь
diagnostic signal, не блокер. Possible causes:

- **Boundary expansion 1980s-2010s** — границы заповедника пересматривались
  несколько раз; documented value 8810 km² может относиться к одной из ранних редакций
- **Polygon simplification** (Nominatim `polygon_threshold=0.005`) — typical 1-2%
  area distortion, даёт ~150-300 km² ошибку
- **Телецкое озеро акватория** — Алтайский включает значительную часть Телецкого озера
  (≈ 230 km² в боковой долине). Inclusion/exclusion lake surface **семантически
  matters для climatology** — water surface vs forest вносят разные TROPOMI XCH₄
  signatures (water reflective albedo, transient ice cover, etc.)

**To be investigated в P-01.0a Алтайский QA test (Algorithm §11.4):** seasonal
cycle сравнение vs Кузнецкий Алатау определит, использовать ли Алтайский
после QA, и если да — может быть нужно re-clip polygon на forest-only mask
через MODIS Land Cover.
...
## Юганский: useable area & escalation gate (clarified 2026-04-27)

Юганский имеет наибольший internal buffer (10 km) среди всех zones — это
обусловлено proximity к oil&gas инфраструктуре по южной границе (Самотлор,
Salym, западные ХМАО facilities). 10 km buffer защищает от downwind advection
из этих sources при unfavourable wind direction.

После apply 10 km buffer:
- `area_km2_total` (documented): 6500 km²
- `area_km2_measured` (polygon area): 6253 km²
- `area_km2_useable` (after 10 km buffer): **2946 km²**

Это ≈ 60 TROPOMI pixels at 7 km grid scale. **Initial concern** в research
phase: per-pixel climatology с count < 100 valid CH4 obs/pixel-month — risky.

**Revised escalation gate (2026-04-27):** Reference Baseline construction
(Algorithm §3.4.0) использует **zone-aggregate** approach (single baseline
value per month per zone, не per-pixel). Total observations contributing
к monthly aggregate:
```
60 pixels × ~30 valid obs/pixel-month × 6 years (2019-2024) ≈ 10,800 obs/month
```
Достаточно robust для zone-aggregate. **Revised threshold: minimum 1000
total observations per month per zone** — все 4 zones easily pass.

Per-pixel climatology threshold (count ≥ 100 obs/pixel-month) сохраняется
как secondary check для regional climatology (Algorithm §3.4.1), не для
reference baseline.

**Internal buffer 10 km для Юганского — KEEP** (защита от advection
критична, нельзя жертвовать ради count). Если P-01.0a покажет zone-aggregate
count < 1000 obs/pixel-month — escalate, не reduce buffer самостоятельно.

См. также OpenSpec entry «Юганский escalation gate clarification» (minor
sub-implementation refinement, не CHANGE).

## Visual Verification

All four zone boundaries visually inspected by principal on 2026-04-27 against
Russian Wikipedia maps, OOPT info maps, regional landscape features. Confirmed
consistent with documented locations and shapes.
